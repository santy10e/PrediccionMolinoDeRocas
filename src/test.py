    ...
    for root, dirs, files in os.walk(data_path):
        for file in files:
            if file.endswith('.wav'):
                ...
                st.write(f"Procesando archivo: {file_path}")  # Mensaje de depuración adicional
                try:
                    ...
                    if 'anormal' in file.lower() or 'abnormal' in file.lower():
                        ...
                        st.write(f"Etiqueta asignada: anormal para archivo {file}")
                    elif 'normal' in file.lower():
                        y_train.append('normal')
                        st.write(f"Etiqueta asignada: normal para archivo {file}")
                    else:
...
if st.button("Reproducir audio normal"):
    audio_normal_file = '/mnt/data/section_00_source_train_normal_0062.wav'
    st.audio(audio_normal_file, format='audio/wav')

if st.button("Reproducir audio anormal"):
    audio_anormal_file = '/mnt/data/section_00_source_train_anormal_0061.wav'
